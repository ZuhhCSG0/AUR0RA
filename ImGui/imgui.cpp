...
void ImGui::Render()
{
	...
	if (g.Style.Alpha > 0.0f)
	{
		...
		if (g.IO.MouseDrawCursor)
		{
			...
			g.OverlayDrawList.AddImage(tex_id, pos, pos + size, cursor_data.TexUvMin[1], cursor_data.TexUvMax[1], IM_COL32(244, 67, 54, 240));       // Black border
			g.OverlayDrawList.AddImage(tex_id, pos, pos + size, cursor_data.TexUvMin[0], cursor_data.TexUvMax[0], IM_COL32(33, 33, 33, 240)); // White fill
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags, ImVec4 color)
{
	...
	if (decimal_precision > 0)
		...
		grab_sz = ImMin(ImMax(1.0f * (slider_sz / ((v_min < v_max ? v_max - v_min : v_min - v_max) + 2.0f)), style.GrabMinSize), slider_sz);  // Integer sliders, if possible have the grab size represent 1 unit
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (*v)
	{
		...
		const float pad = ImMax(1.0f, (float)(int)(check_sz / 9.0f));
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
	...
	RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
	//RenderFrame(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, false, style.FrameRounding); // FIXME-ROUNDING
