...
namespace hooks
{
    ...
    void initialize()
    {
		...
		m_present = U::pattern_scan(GetModuleHandle(XorStr("gameoverlayrenderer.dll")), XorStr("57 50 FF 75 0C 56 FF 15 ? ? ? ? 8B F8 85")) + 0x8;
		m_reset = U::pattern_scan(GetModuleHandle(XorStr("gameoverlayrenderer.dll")), XorStr("57 C7 45 FC ? ? ? ? FF 15 ? ? ? ? 8B F8 85")) + 0xA;
